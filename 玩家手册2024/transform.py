...
def deal_with_klass_line(klass_line: str) -> list[KlassWithExtraInfo]:
    # 先根据“；”分割
...
def transform_spell_from_html_block(html_block: list[PageElement]) -> Spell:
    ...
    # 将html_block的第三个标签后的所有contents拼接起来作为法术详情
    details = []
    ...
    class_list = deal_with_klass_line(first_tag_text[2].strip())
    ...
    if '或仪式' in casting_time:
        is_ritual = True
        casting_time = casting_time.replace('或仪式', '').strip()
    else:
        is_ritual = False
    current_index += 1
...
def transform() -> list[Spell]:
    import os
    print("[Module] 正在转化：玩家手册2024")
    # 遍历 玩家手册/raw 目录下的所有文件
    raw_dir = os.path.join(os.path.dirname(__file__), "raw")
    # 获取所有 HTML 文件的路径列表
    filename_list = [os.path.join(raw_dir, filename) for filename in os.listdir(raw_dir) if filename.endswith('.htm')]
