...
def transform_spell_from_html_block(html_block: list[PageElement]) -> Spell:
    ...
    for index in range(1, len(details)):
        tag_text = details[index].get_text().strip()
        if tag_text == "Legacy":
            is_legacy = True
            break
    else:
    ...
    while current_index < len(details):
        ...
        if tag_text.startswith("施法时间："):
            tag_text = tag_text.replace("施法时间：", "").strip()
            if tag_text != "":
                casting_time = tag_text
            else:
                current_index += 1
    ...
    while current_index < len(details):
        ...
        if tag_text.startswith("施法距离："):
            tag_text = tag_text.replace("施法距离：", "").strip()
            if tag_text != "":
                spell_range = tag_text
            else:
                current_index += 1
    ...
    while current_index < len(details):
        ...
        if tag_text.startswith("法术成分："):
            tag_text = tag_text.replace("法术成分：", "").strip()
            if tag_text != "":
                need_verbal, need_somatic, material = deal_with_material_line(tag_text)
            else:
                current_index += 1
    ...
    while current_index < len(details):
        ...
        if tag_text.startswith("持续时间："):
            tag_text = tag_text.replace("持续时间：", "").strip()
            if tag_text != "":
                need_concentration, duration = deal_with_duration_line(tag_text)
            else:
                current_index += 1
...
def transform() -> list[Spell]:
    import os
    print("[Module] 正在转化：塔莎的万事坩埚")
    # 遍历 玩家手册/raw 目录下的所有文件
    raw_dir = os.path.join(os.path.dirname(__file__), "raw")
    # 获取所有 HTML 文件的路径列表
    filename_list = [os.path.join(raw_dir, filename) for filename in os.listdir(raw_dir) if filename.endswith('.html')]
